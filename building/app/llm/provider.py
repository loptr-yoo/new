    ...
    async def chat(self, messages: List[ChatMessage], config: LLMConfig) -> str:
        ...
        try:
            ...
            if finish_reason and finish_reason != "STOP":
                partial_text = ""
                try:
                    parts0 = candidate.get("content", {}).get("parts", [])
                    if parts0:
                        partial_text = str(parts0[0].get("text") or "")
                except Exception:
                    partial_text = ""
                if finish_reason == "SAFETY":
                    raise ValueError("Content blocked by safety filter. finishReason: SAFETY")
                raise ValueError(
                    f"Gemini response did not finish cleanly: finishReason={finish_reason}, "
                    f"partial_response_chars={len(partial_text)}"
                )
