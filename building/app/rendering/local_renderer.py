...
try:
    from building.app.geometry.style_constants import SEGMENTATION_COLORS  # type: ignore[import-not-found]
except Exception:
    SEGMENTATION_COLORS = {}
...
def render_floor_layout(layout: Dict[str, Any], out_path: Path, mode: str = "seg") -> None:
    """Render one floor layout with the backend local renderer."""
    _render(layout, out_path, mode)


def render_building_floors(building_dict: Dict[str, Any], out_dir: Path, modes: List[str]) -> Dict[str, str]:
    """Write per-floor layout JSON and render artifacts for each requested mode."""
    building = building_dict.get("building") if isinstance(building_dict, dict) else None
    floors = building.get("floors") if isinstance(building, dict) else None
    if not isinstance(floors, dict):
        raise ValueError("building_dict must contain building.floors")

    out_dir.mkdir(parents=True, exist_ok=True)
    artifacts: Dict[str, str] = {}
    for floor_id in sorted(str(k) for k in floors.keys()):
        layout = floors.get(floor_id)
        if not isinstance(layout, dict):
            continue
        suffix = floor_id if floor_id.upper().startswith("F") else f"F{floor_id}"
        layout_path = out_dir / f"layout_{suffix}.json"
        layout_path.write_text(json.dumps(layout, ensure_ascii=False, indent=2), encoding="utf-8")
        artifacts[f"layout_{suffix}"] = str(layout_path)
        for mode in modes:
            render_path = out_dir / f"render_{suffix}_{mode}.png"
            render_floor_layout(layout, render_path, mode=mode)
            artifacts[f"render_{suffix}_{mode}"] = str(render_path)
    return artifacts
